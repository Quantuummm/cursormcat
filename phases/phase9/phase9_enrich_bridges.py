...
Creates Bridge Missions — cross-planet connections with explanations and bridge questions.
Aligned with MCAT Mastery Game Design v2.
...
from config import EXTRACTED_DIR, BRIDGES_DIR, BOOKS, LORE_DIR
from utils.gemini_client import GeminiClient


# ─── MCAT Real Cross-Subject Patterns ──────────────────────
# These are actual MCAT testing patterns that go BEYOND Kaplan's shared concepts.
# Injected into bridge enrichment prompts for higher-quality connections.
MCAT_CROSS_SUBJECT_PATTERNS = [
    {"subjects": ["biology", "gen_chem"], "pattern": "Acid-base chemistry in biological buffers (blood pH, Henderson-Hasselbalch in renal physiology)"},
    {"subjects": ["biology", "physics"], "pattern": "Fluid dynamics in cardiovascular system (Bernoulli's, Poiseuille's law in blood flow)"},
    {"subjects": ["biology", "biochemistry"], "pattern": "Enzyme kinetics in metabolic pathways (Michaelis-Menten applied to real enzymes)"},
    {"subjects": ["biochemistry", "gen_chem"], "pattern": "Thermodynamics of metabolic reactions (Gibbs free energy in glycolysis, electron transport)"},
    {"subjects": ["biochemistry", "org_chem"], "pattern": "Reaction mechanisms in enzyme catalysis (nucleophilic attack, acid-base catalysis)"},
    {"subjects": ["physics", "biology"], "pattern": "Optics and vision (lens equations applied to the eye, corrective lenses)"},
    {"subjects": ["physics", "gen_chem"], "pattern": "Electrochemistry and cell potentials (Nernst equation, galvanic vs electrolytic)"},
    {"subjects": ["psych_soc", "biology"], "pattern": "Nervous system and behavior (neurotransmitter pathways, drug mechanisms)"},
    {"subjects": ["psych_soc", "biochemistry"], "pattern": "Hormonal regulation of behavior (cortisol, dopamine pathways)"},
    {"subjects": ["gen_chem", "org_chem"], "pattern": "Acid-base chemistry across contexts (pKa, nucleophilicity, leaving groups)"},
    {"subjects": ["biology", "psych_soc"], "pattern": "Sensation and perception (signal transduction to psychological processing)"},
    {"subjects": ["physics", "biochemistry"], "pattern": "Spectroscopy in molecular analysis (UV-Vis, NMR applied to biomolecules)"},
]
...
def run():
    ...
    prompt_template = (Path(__file__).parent / "bridge_enrichment.txt").read_text(encoding="utf-8")
