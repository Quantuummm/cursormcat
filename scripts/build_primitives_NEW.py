"""\
Build Primitives (utility script)
--------------------------------
Creates game primitives from existing extraction outputs.
...
This script is intentionally not numbered as a pipeline phase to avoid
renumbering Phase 10/11 (TTS/Firestore). Phase 12 (compile modes) will
also auto-build primitives if missing.
"""
...
from config import EXTRACTED_DIR, PRIMITIVES_DIR, BOOKS
    ...
    for subject in subjects:
        ...
        for ch_path in chapter_files:
            ...
            for section in ch_data.get("sections", []) or []:
                ...
                print(
                    f"  ✅ {sec_id}: terms={prim['signals']['term_count']} "
                    f"processes={prim['signals']['process_count']} "
                    f"tables={prim['signals']['table_count']} figs={prim['signals']['figure_count']}"
                )
